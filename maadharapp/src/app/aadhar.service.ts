...
//==============================ADMIN: LOGIN REGISTER LOGOUT=============================================//
  ...
  public adminLogout(email: string): Observable<string> {
    return this.http.put<string>(
      `http://localhost:8080/admin/logout/${email}`,
      null
    );
  }

  public registerAdmin(admin: any): Observable<string> {
    return this.http.post<string>(`http://localhost:8080/admin/`, admin);
  }
  ...
  public checkCitizenLoginCreds(
    ...
    return this.http.put<string>(
      `http://localhost:8080/citizen/login/${email}/${password}`,
      null
    );
  }

  public getCitizen(email: string): Observable<any> {
    return this.http.get(`http://localhost:8080/citizen/email/${email}`);
  }
  ...
  public citizenLogout(email: string): Observable<string> {
    return this.http.put<string>(
      `http://localhost:8080/citizen/logout/${email}`,
      null
    );
  }
  public registerCitizen(citizen: any): Observable<string> {
    return this.http.post<string>(`http://localhost:8080/citizen/`, citizen);
  }
  
  //================================ADMIN OPERATIONS ON CITIZENS====================================//

  public getWaitingList(): Observable<any> {
    return this.http.get('http://localhost:8080/admin/waiting');
  }
  public approve(email: string): Observable<any> {
    return this.http.put(`http://localhost:8080/admin/approve/${email}`, null);
  }
  public reject(email: string): Observable<any> {
    return this.http.put(`http://localhost:8080/admin/reject/${email}`, null);
  }
  public updateDeadCitizen(email:string):Observable<String>{
    return this.http.put<string>(`http://localhost:8080/admin/notAlive/${email}`, null);
  }
}